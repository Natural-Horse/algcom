...
int qr_pre(int l, int r, int val){
    int fab = 0; bool check = false;
    if(bl[l] == bl[r]){
        for(R int i = l; i <= r; i++) h[cl[a[i]]]++, p[a[i]]++;
        for(R int i = val-1; i >= (cl[val]-1)*L+1; --i) if(p[i]){fab = i, check = true; break;}
        if(!fab) for(R int i = cl[val]-1; i; --i) if(h[i]){fab = i; break;}
        if(!check && fab) for(R int i = fab*L; i >= (fab-1)*L+1; --i) if(p[i]){fab = i; break;}
        for(R int i = l; i <= r; i++) h[cl[a[i]]]--, p[a[i]]--;
        if(!fab) return 1;
        else return fab;
    }
        ...
        for(R int i = l; i <= bl[l]*S; i++) h[cl[a[i]]]++, p[a[i]]++;
        for(R int i = (bl[r]-1)*S+1; i <= r; i++) h[cl[a[i]]]++, p[a[i]]++;
        for(R int i = val-1; i >= (cl[val]-1)*L+1; --i) if(p[i]+g[bl[r]-1][i]-g[bl[l]][i]){fab = i, check = true; break;}
        if(!fab) for(R int i = cl[val]-1; i; --i) if(h[i]+f[bl[r]-1][i]-f[bl[l]][i]){fab = i; break;}
        if(!check && fab) for(R int i = fab*L; i >= (fab-1)*L+1; --i) if(p[i]+g[bl[r]-1][i]-g[bl[l]][i]){fab = i; break;}
        for(R int i = l; i <= bl[l]*S; i++) h[cl[a[i]]]--, p[a[i]]--;
        for(R int i = (bl[r]-1)*S+1; i <= r; i++) h[cl[a[i]]]--, p[a[i]]--;
        if(!fab) return 1;
        else return fab;
...
int qr_suf(int l, int r, int val){
    ...
    if(bl[l] == bl[r]){
        for(R int i = l; i <= r; i++) h[cl[a[i]]]++, p[a[i]]++;
        for(R int i = val+1; i <= cl[val]*L; i++) if(p[i]){fab = i, check = true; break;}
        if(!fab) for(R int i = cl[val]+1; i <= C; i++) if(h[i]){fab = i; break;}
        if(!check && fab) for(R int i = (fab-1)*L+1; i <= min(num, fab*L); i++) if(p[i]){fab = i; break;}
        for(R int i = l; i <= r; i++) h[cl[a[i]]]--, p[a[i]]--;
        if(!fab) return num;
        else return fab;
    }
        ...
        for(R int i = val+1; i <= cl[val]*L; i++) if(p[i]+g[bl[r]-1][i]-g[bl[l]][i]){fab = i, check = true; break;}
        if(!fab) for(R int i = cl[val]+1; i <= C; i++) if(h[i]+f[bl[r]-1][i]-f[bl[l]][i]){fab = i; break;}
        if(!check && fab) for(R int i = (fab-1)*L+1; i <= min(num, fab*L); i++) if(p[i]+g[bl[r]-1][i]-g[bl[l]][i]){fab = i; break;}
        for(R int i = l; i <= bl[l]*S; i++) h[cl[a[i]]]--, p[a[i]]--;
        for(R int i = (bl[r]-1)*S+1; i <= r; i++) h[cl[i]]--, p[a[i]]--;
        if(!fab) return num;
        else return fab;
...
int main(){
    ...
    for(int i = 1; i <= n; i++){
        bl[i] = (i-1) / S + 1;
        f[bl[i]][cl[a[i]]] = f[bl[i]-1][cl[a[i]]] + 1;
        g[bl[i]][a[i]] = g[bl[i]-1][a[i]] + 1;
    }
//    printf("num:%d\n", num);
//    printf("S:%d P:%d L:%d C:%d\n", S, P, L, C);
    for(int i = 1; i <= m; i++){
        int op = q[i].op;
        if(op == 1){
